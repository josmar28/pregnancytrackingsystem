...
export default {
      ...
      this.$refs.form
        ...
        .then((success) => {
          if (success) {
            ...
            ["name", "username", "email", "mobile", "status"].forEach(
              (field) => {
            ...
            formData.append(
              "role_ids",
              JSON.stringify(this.form.role_ids?.map((item) => item.id))
            );
            ...
            this.$http
              .post("/api/users" + (this.id ? `/${this.id}` : ""), formData)
              .then(({ data }) => {
                this.softToast(
                  "success",
                  this.$t("Success"),
  ...
  mounted() {
    this.$store.dispatch(SET_BREADCRUMB, [
      ...
      {
        text: this.id
          ? this.$t("modules.users.edit")
          : this.$t("modules.users.create"),
        active: true,
      },
    ]);
  },
